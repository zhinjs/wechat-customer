...
- **首次启动**：提供 `channelToken` + `jwtToken`（QClaw）或 `accessToken`（WorkBuddy），`connect()` 成功后：
  - QClaw：自动保存 guid 到 `device.json`，保存完整凭证到 `session.json`
  - WorkBuddy：自动保存完整凭证到 `session.json`
- **后续启动**：不传任何 `credentials`，`connect()` 自动从本地文件恢复，无需任何额外参数。
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `hasSavedSession()` | `() => Promise<boolean>` | 检查是否有已保存的会话文件（session.json） |
| `loadSavedCredentials()` | `() => Promise<ChannelCredentials \| null>` | 读取已保存的凭证（不建立连接） |
| `clearSession()` | `() => Promise<void>` | 删除会话文件（下次需要重新传入凭证；不影响 device.json） |
| `hasSavedDevice()` | `() => Promise<boolean>` | 检查是否有已保存的设备 GUID（device.json，QClaw 专用） |
| `clearDevice()` | `() => Promise<void>` | 删除设备文件（下次生成新 GUID，相当于重置设备标识） |
...
// 不传任何凭证，SDK 自动恢复 device.json + session.json
const sdk = new WeChatSDK();
await sdk.connect();
...
if (!hasSession) {
  ...
  console.error('QClaw:     QCLAW_CHANNEL_TOKEN, QCLAW_JWT_TOKEN');
...
if (creds?.mode === 'qclaw') {
  ...
  console.log('设备 GUID:', creds.guid);        // 自动生成或从 device.json 恢复
...
- 首次连接：自动生成 UUID 并保存到 `~/.wechat-sdk/device.json`
